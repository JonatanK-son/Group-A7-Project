...
# 3. Apply Spark RBAC (ServiceAccount, Role, Binding) and Job
echo -e "\033[0;32mApplying Spark RBAC and Job Configuration...\033[0m"
...
sleep 5 # wait for job controller to create pod
kubectl wait --for=condition=Initialized pod -l job-name=spark-pipeline-job --timeout=60s
...
kubectl logs -f job/spark-pipeline-job --all-containers=true
